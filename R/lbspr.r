...
# # Generate Length Frequencies 
# NYears <- 6
# Ns <- ceiling(runif(NYears, 150, 300))
# LenFreq <- rmultinom(NYears, size=Ns, prob=LB_obj@pLCatch[,1])
# LenFreq <- cbind(LB_obj@LMids, LenFreq)

# # Generate Raw Length Data 
# LB_pars@BinWidth <- 1 
# LB_obj2 <- LBSPRsim(LB_pars, Control=list(modtype="GTG"))
# LenRaw <- matrix(NA, nrow=max(Ns), ncol=NYears)
# for (X in 1:NYears) {
  # for (r in 1:Ns[X]) {
    # LenRaw[r,X] <- sample(LB_obj2@LMids, size=1, prob=LB_obj2@pLCatch[,1])
  # }
# }
# par(mfrow=c(1,2))
# hist(LenRaw[,1], breaks=LB_obj@LMids, col="gray")
# barplot(LenFreq[,2], names.arg=LenFreq[,1])


# .Object <- new("LB_lengths")

# setwd("E:/GitRepos/shiny-server/lbspr/data")
# LB_lengths <- new("LB_lengths", LB_pars=LB_pars, file="LFreq_MultiYrHead.csv", 
